...
void write_inode_bitmap(int fd)
{
    ...
    int num_reserved_inodes=EXT2_GOOD_OLD_FIRST_INO-1;
    int reserved_indexes=num_reserved_inodes/8;
    //set reserved bits to 1
    for(int i=0;i<reserved_indexes;i++){
        map_value[i]=0b11111111;
    }
    int next_byte_left_over=num_reserved_inodes%8;
    for(int i=0;i<next_byte_left_over;i++){
        map_value[reserved_indexes]|=1<<i;
    }


    //start with the first good inode in our case 11
    const int num_good_inodes=LAST_INO-EXT2_GOOD_OLD_FIRST_INO+1;
    int good_inodes[LAST_INO-EXT2_GOOD_OLD_FIRST_INO+1]={LOST_AND_FOUND_INO,HELLO_WORLD_INO,HELLO_INO};
    for(int i=0;i<num_good_inodes;i++){
        int inode_to_bit=good_inodes[i]-1;//because inodes start at num 1. ith bit maps to i+1th inode
        int byte=inode_to_bit/8;
        int bit_offset=inode_to_bit%8;
        map_value[byte]|=1<<bit_offset;
    }
    int last_indexed_used;
    if(NUM_INODES%8==0){
        last_indexed_used=NUM_INODES/8-1;
    }
    else{
        last_indexed_used=NUM_INODES/8;
    }
    for (int i=last_indexed_used+1;i<BLOCK_SIZE;i++){
        map_value[i]=0b11111111;
    }
...
void write_inode_table(int fd) {
    ...
    hello_inode.i_blocks=0;// No additional blocks are allocated
    // Fast symbolic link: store the target path directly in i_block
...
void write_hello_world_file_block(int fd)
{
    ...
    const char* HELLO_WORLD_FILE_CONTENT = "Hello world\n";
        ssize_t size = strlen(HELLO_WORLD_FILE_CONTENT);
    if (write(fd, HELLO_WORLD_FILE_CONTENT , size) != size) {
        errno_exit("write");
    }
}
